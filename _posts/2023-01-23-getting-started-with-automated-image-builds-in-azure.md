...
*  **Consistency**. The image build process is the same every time. Code is Code. Execution of code and associated configurations are the same every time. This results in `human error reduction` and guaranteed repeatablity
*  **Flexibility**. Building on the the consistency logic, there is flexibility of building the same thing, consistently, on multiple platftorms. With some tools, it is relatively simple to deploy the exact some base, across many different infrastructure providers. It blows my mind that by simply changing 8 or 9 attributes in a packer build script, we can effectively (within reason of course) build an identical image on a different cloud platform or hypervisor.
